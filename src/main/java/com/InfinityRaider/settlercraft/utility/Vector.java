...
public class Vector {
    ...
    public Vector(NBTTagCompound tag) throws UnknownPositionException {
        if(!tag.hasKey(Names.NBT.X1)) {throw new UnknownPositionException();}
        if(!tag.hasKey(Names.NBT.Y1)) {throw new UnknownPositionException();}
        if(!tag.hasKey(Names.NBT.Z1)) {throw new UnknownPositionException();}
        this.x = tag.getDouble(Names.NBT.X1);
        this.y = tag.getDouble(Names.NBT.Y1);
        this.z = tag.getDouble(Names.NBT.Z1);
    }

    public NBTTagCompound writeToNBT() {
        NBTTagCompound tag = new NBTTagCompound();
        tag.setDouble(Names.NBT.X1, x);
        tag.setDouble(Names.NBT.Y1, y);
        tag.setDouble(Names.NBT.Z1, z);
        return tag;
    }
